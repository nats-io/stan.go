...
func main() {
	...
	flag.StringVar(&clientID, "id", defaultID, "The STAN client ID to connect with")
	flag.StringVar(&clientID, "clientid", defaultID, "The STAN client ID to connect with")
	...
	sub, err := sc.Subscribe(subj, mcb, startOpt, DurableName(durable))
	if err != nil {
		sc.Close()
		log.Fatal(err)
	}
